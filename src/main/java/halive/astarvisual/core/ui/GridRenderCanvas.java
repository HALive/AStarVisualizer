...
import halive.astarvisual.core.mapgenerator.MapGenerator;
import halive.astarvisual.core.mapgenerator.MapGenerators;
...
public class GridRenderCanvas extends ActiveRenderingCanvas {
    ...
    private MapGenerators mapGenerator = MapGenerators.values()[0];
    ...
    public GridRenderCanvas(Container frame) {
        ...
        for (int x = 0; x < tiles.length; x++) {
            for (int y = 0; y < tiles[x].length; y++) {
                tiles[x][y] = new Tile(x * AStarVisualizer.SQUARE_SIZE,
                        y * AStarVisualizer.SQUARE_SIZE, Tile.NodeState.UNKNOWN, this);
            }
        }
        clear();
    }
    ...
    public void draw(Graphics g) {
        ...
        g.drawString("Heuristik: " + mode.toString() + " Kartengenerator: " + mapGenerator.toString(),
                5, (int) (15 + (tiles[0].length * AStarVisualizer.SQUARE_SIZE * yScale)));
        g.drawString("Tasten: H = Heuristik ändern; S = Startknoten setzen;" +
                        " E = Endknoten Setzen; C = Fläche bereinigen; R = Zufallskarte Erzeugen; G = Kartengenerator wählen",
                5, (int) (30 + (tiles[0].length * AStarVisualizer.SQUARE_SIZE * yScale)));
    ...
    private void changeGenerator() {
        if (allowDrawing) {
            genPos = (genPos + 1) % MapGenerators.values().length;
            mapGenerator = MapGenerators.values()[genPos];
        }
    }
    ...
    private void generateRandomMap() {
        if (allowDrawing) {
            ...
            MapGenerator generator = mapGenerator.getGenerator();
            boolean[][] map = generator.generateMap(xMax, yMax);
            for (int x = 0; x < xMax; x++) {
                for (int y = 0; y < yMax; y++) {
                    Tile.NodeState s = !map[x][y] ? Tile.NodeState.WALL : Tile.NodeState.UNKNOWN;
            ...
            double dist = 0;
            do {
                startPos = generator.getStartPos();
                endPos = generator.getEndPos();
